...
1. *python bin/findThresholds-QCplots.py outfile_rootname.scartab.tsv* <br/>
 ...
  * ![sumXscar.fit](https://github.com/anna-alemany/scScarTrace/tree/master/examples/sumXscar.fit): text file with results from fitting the histogram of total number of reads per scar to a three domain piecewise-defined linear function. The third domain corresponds to scars that are highly observed and therefore are real scars; the second domain contains scars that generated because of sequencing errors, therefore these are closely related to real scars but get a different cigar barcode assigned; the first domain corresponds to scars that are hadrly observed and therefore are generated by sequencing noise. The border between the second and the third domain _y_ provides a reasonable threshold to filter out scars that are seen with less that _10^y_ reads. 
...
2. *python bin/filter-normalize.py outfile_rootname.scartab.tsv 10^x 10^y outfile_rname*
